...
aperture_i = ePIE_inputs(1).InitialAp;
iterations = ePIE_inputs(1).Iterations;
update_aperture = ePIE_inputs(1).updateAp;
...
if isfield(ePIE_inputs, 'FileName');
...
freeze_aperture = Inf;
optional_args = {0.9,0.1,0.5, 0, 0.1,0.4,4, 0}; %default values for varargin parameters
nva = length(varargin);
...
positions_y = unique(positions(:,2));
nProbe = ceil(length(positions_y)/10);
fprintf('Number of local probes = %d\f',nProbe);
index_probe = zeros(nApert,1);
for n=1:nProbe
    logical_A = positions(:,2)>=positions_y((n-1)*10+1) & ...
        positions(:,2)<=positions_y(min(n*10,length(positions_y)));    
    index_probe(logical_A) = n;
end
...
if aperture_i == 0
    aperture_i = single( 2*makeCircleMask(round(aperture_radius./pixel_size),little_area) );
    aperture_i = my_ifft(aperture_i);
    initial_aperture = aperture_i;
    aperture = repmat(aperture_i, [1,1,nProbe]);
else
    aperture = repmat(single(aperture_i), [1,1,nProbe]);
...
fprintf('big_obj size = %dx%d, aperture size = %dx%dx%d, pixel_size = %f\n',size(big_obj), size(aperture),pixel_size);
...
for t = 1:iterations
    ...
    for aper = randperm(nApert)
        ...
        if t>=5
            aperture_i = aperture(:,:, index_probe(aper));
        end        
        u_old = big_obj(Y1(aper):Y2(aper), X1(aper):X2(aper));
        %object_max = max(abs(u_old(:))).^2;
        probe_max = max(abs(aperture_i(:))).^2;
        %% Create new exitwave
        Pu_old = u_old.*(aperture_i);
        z_u = fft2(Pu_old);
        check_dp = abs(z_u);
        ...
        u_temp = ( ((1-beta_obj)/dt)*u_old + Pu_new.*conj(aperture_i)) ./ ( (1-beta_obj)/dt + abs(aperture_i).^2 );
        %u_temp = imgaussfilt(abs(u_temp),0.2*(iterations-t+1)/iterations).* exp(1j*angle(u_temp));
        %u_temp = imgaussfilt(real(u_temp),0.2*(iterations-t+1)/iterations) + ...
        %      1j*imgaussfilt(imag(u_temp),0.2*(iterations-t+1)/iterations);
        ...
        if update_aperture == 1
            ...
            if semi_implicit_P
                aperture_i = ((1-new_beta_ap)*aperture_i + ds*Pu_new.*conj(u_old)) ./ ( (1-new_beta_ap) + ds*abs(u_old).^2 );
            else
                aperture_i = aperture_i - ds*conj(u_old).*(diff);
            end
            
            if t>=5
                aperture(:,:, index_probe(aper)) = aperture_i;
            end            
            
            %aperture = imgaussfilt(real(aperture),0.2*sqrt((iterations-t+1)/iterations)) + ...
            %        1j*imgaussfilt(imag(aperture),0.2*sqrt((iterations-t+1)/iterations));
        end
        if sum_dp(aper)
            fourier_error(t,aper) = sum(sum(abs( check_dp(~missing_data) - diffpat_i(~missing_data) ))) ./ sum_dp(aper);
        end        
    end
    %big_obj = imgaussfilt(abs(big_obj),0.4*(iterations-t+1)/iterations).* exp(1j*angle(big_obj));
    
    if probeNorm
        scale = max(max(abs(aperture_i)));
        aperture_i = aperture_i/scale;
    end
    %{
    if t>iterations-100
...
%         drawnow
        ...
        figure(33); img(croppedOut(abs(big_obj),[cropR+100, cropC+100]),num2str(t), 'colormap', 'gray');
        %figure(34); imagesc(hsv_ap); axis image; colormap('gray'), title(num2str(t));
        drawnow            
    end
    ...
    if save_intermediate == 1 && mod(t,100) == 0
        inter_frame = inter_frame + 1;
        save(['inter_output_DR_',FileName,'iter_',num2str(t),'.mat'],...
