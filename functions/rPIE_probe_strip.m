...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture, images] = rPIE_probe_strip(ePIE_inputs,varargin)
...
width = ePIE_inputs(1).strip_width;
...
nProbe = ceil(length(positions_y)/width);
fprintf('Number of sub-probes = %d\n',nProbe);
index_probe = zeros(nApert,1);
for n=1:nProbe
    logical_A = positions(:,2)>=positions_y((n-1)*width+1) & ...
        positions(:,2)<=positions_y(min(n*10+1,length(positions_y)));    
...
if aperture_i == 0
    ...
    %aperture_i = single( 2*makeCircleMask(64,little_area) );
...
%% Create new exitwave
        buffer_exit_wave = rspace.*(aperture_i);
...
%% Update the object
        ...
        new_rspace = rspace + conj(aperture_i).*diff_exit_wave ./ ...
            ((1-beta_obj).*abs(aperture_i).^2 + beta_obj.*probe_max);
        if do_posi == 1
...
%% Update the probe
        ...
        if update_aperture == 1
            ...
            aperture_i = aperture_i +update_factor_pr*conj(rspace).*(diff_exit_wave);
            %aperture_i = imgaussfilt(real(aperture_i),0.2*sqrt((iterations-itt+1)/iterations)) + ...
            %          1j*imgaussfilt(imag(aperture_i),0.2*sqrt((iterations-itt+1)/iterations));
            if itt>=5
...
%         drawnow 
        ...
        figure(33); img((abs(big_obj)),num2str(itt), 'colormap', 'gray');
