...
Kernel = ePIE_inputs(1).Fmask;
norm_K = norm(Kernel,'fro');
...
for itt = 1:iterations
    ...
    for aper = randperm(nApert)
        ...
        obj = big_obj(Y1(aper):Y2(aper), X1(aper):X2(aper));
        object_max = max(abs(obj(:))).^2;
        probe_max = max(abs(aperture(:))).^2;
        Kernel_max = max(abs(Kernel(:))).^2;

        %% Create new exitwave
        Pu_old = obj.*(aperture);
        dp_old = fft2(Pu_old)  .* Kernel;
        check_dp = abs(dp_old);
        current_dp = diffpats(:,:,aper);
        missing_data = current_dp == -1;
        k_fill = dp_old(missing_data);
        dp_new = current_dp.*exp(1i*angle(dp_old)); % Replace reconstructed magnitudes with measured magnitudes
        dp_new(missing_data) = k_fill;
        
        %% Update the object
        diff_dp = (dp_new-dp_old);
        diff_exit_wave = ifft2(diff_dp .* conj(Kernel));
        dt = beta_obj/probe_max/Kernel_max;
        obj_new = obj + dt.*conj(aperture).*(diff_exit_wave);
        if do_posi == 1
            obj_new = max(0, real(obj_new));
        end
        big_obj(Y1(aper):Y2(aper), X1(aper):X2(aper)) = obj_new;
        %% Update the probe
        
        if update_aperture == 1 && itt<30
            if itt > iterations - freeze_aperture
                new_beta_ap = beta_ap*sqrt((iterations-itt)/iterations);
                ds = new_beta_ap./object_max/Kernel_max;
            else
                ds = beta_ap./object_max/Kernel_max;
            end
            aperture = aperture +ds*conj(obj).*(diff_exit_wave);
        end
        ...
        %% update kernel
        if itt>inf
            max_Pu = max(abs(Pu_old(:))).^2;
            dk = 0.01;%*sqrt((iterations-itt)/iterations);
            Kernel = Kernel + (dk/max_Pu/size(Kernel,1)^2)*(diff_dp).* conj(fft2(Pu_old));
            Kernel = Kernel/norm(Kernel,'fro')*norm_K;
        end

    end
    if itt<30,
        big_obj = imgaussfilt(real(big_obj),0.5) + 1j*imgaussfilt(imag(big_obj),0.5);
    end
    if itt>40 && mod(itt,10)==3
        if itt==21, mu = 0.03; else mu = 0.003; end
        g_denoise = SB_ATV(double(big_obj(:)),mu);
        big_obj = reshape(g_denoise,size(big_obj));
    end
...
%         drawnow
        ...
        figure(43); img(croppedOut(big_obj,[cropR,cropC]), ['ePIE ', num2str(itt)], 'colormap', 'gray');
        figure(44); imagesc(hsv_ap); axis image; colormap('gray'), title(num2str(itt));
        drawnow          
    end
